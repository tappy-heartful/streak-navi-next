...
- **プレースホルダー必須**: テキスト入力・数値入力（`<AppInput>` / `<input type="text|number">`）には必ず `placeholder` を設定すること。入力例や期待値を簡潔に示す（例: `"例: ¥2,500"`, `"https://..."`, `"タイトルを入力してください"`）
- **アイコン必須**: 画面タイトル（`<h1>`）には Font Awesome アイコン（`<i className="fa-solid fa-...">` 等）を必ず付ける。機能ごとに一貫したアイコンを使うこと（一覧・確認・編集・回答登録で同じアイコン）
- **未設定状態の案内**: ユーザーが選択・入力した値に対してデータが存在しない場合（例: 旅費額未設定）は、`showDialog()` で状況を伝え、管理者なら設定画面への誘導、一般ユーザーには管理者への連絡を促す
- **ダイアログのタイミング**: ダイアログは自動入力・初期値セット時ではなく、ユーザーが明示的に操作（セレクト選択・ボタン押下）したタイミングで出すこと
